...
- **`curl -d '…한글…'` 은 깨진다.** Git Bash 가 CP949 로 보내 JSON 바인딩이 실패하고, 응답은 **본문 없는 400**
  이라 원인이 안 보인다(서버 로그에 `400 0` 으로만 남는다). 한글 본문은 **UTF-8 파일에 담아 `--data-binary @파일`** 로 보낸다.
- **서버가 떠 있으면 DLL 이 잠겨 빌드가 실패한다** → `taskkill //F //IM Harbor.Server.exe` 먼저. (사용자가 띄워 둔 창도 같이 죽으니 알리고 할 것)

**웹앱(`webapp/`)을 고쳤으면 커밋 전에 `npm run build` 를 돌린다.**
`npm run dev` 는 **타입 검사를 건너뛴다** — import 하나 빠져도 로컬에서는 멀쩡히 돌고 **배포만 실패한다**.
그러면 Vercel 은 마지막으로 성공한 배포를 계속 서비스하므로, 화면은 옛날 것 그대로라 알아채기도 어렵다(2026-09-21 실제로 겪음).

```
! cd /c/Users/User/Desktop/harbor/webapp && npm run build 2>&1 | tail -20
```
...
- **물건이 생기는 상호작용(`give_item:`)은 개인 방에서 주인만.** 아니면 남의 방을 돌며 수확해 파는 구멍이 된다.
  공용 방은 예외(무한 자판기가 설계다). 의자·조명처럼 물건이 안 생기는 것은 남의 방에서도 된다.
- **공용 방은 기본이 잠김**(`RoomDef.OpenEdit`). 열려 있으면 아무나 광장 가구를 집어 간다.
