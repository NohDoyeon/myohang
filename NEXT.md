...
**남의 방에 물건을 놓는 권한 예외는 포스트잇과 캣닢뿐이다** — 이게 사교의 두 축이다.
서버는 이미 다 되어 있다:
- `C_PlaceItem` 이 포스트잇은 남의 방에도 허용 (`RoomInstance.cs` 의 `CanEdit` 예외)
- `C_PostitWrite`(0x0050) — **글쓴이 본인만**, 200자 (`OnPostitWrite`)
- `ItemDto.Author`(글쓴이) · `Extra`(본문) · `State`(`blank`/`written`) 가 이미 실려 온다

**웹에서 할 일은 클라이언트 쪽뿐이다**:
1. 캔버스에서 **가구를 클릭**할 수 있게 — 지금은 바닥 타일만 클릭된다(`RoomRenderer.onTileClick`)
2. 포스트잇을 클릭하면 **글쓴이와 본문**을 보여 주기
3. 내가 붙인 빈 포스트잇이면 **쓰기 입력**
4. 벽걸이라 `wallU`/`wallV` 슬롯 배치가 필요할 수 있다 — 우선 바닥 기준으로 놓고 확인
