...
### 하고 싶은 것 — 친구 목록 · 놀러 가기 (2026-09-21 사용자 요청)

지금은 **방 목록의 "집" 섹션**이 그 역할을 대신한다. 저장된 방은 주인이 접속하지 않아도 목록에 남으므로
(`RoomManager.RestoreSaved`) 남의 집에 놀러 가 화분에 캣닢을 꽂아 둘 수 있다 — 원래 설계한 사교 고리다.

제대로 만들려면 **서버 작업이 필요하다**:
- `friend` 테이블 (추가·수락·삭제) — 소유권은 **게임 서버**(`player` 계열과 같이 간다)
- 친구가 **지금 접속해 있는지** — `OnlineUsers` 가 이미 세션을 들고 있으니 조회는 쉽다
- 친구가 있는 방으로 **바로 가기**
- 프로토콜 `S_RelationUpdate`(0x1090) 자리가 비어 있다 — 결혼·짝짓기까지 염두에 두고 잡아 둔 번호

먼저 할 값어치가 있는 이유: **접속했을 때 "누가 있나"를 아는 것**이 이 장르의 핵심이다.
지금은 방 목록을 열어 초록색 숫자를 눈으로 찾아야 한다.
