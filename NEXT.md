...
### ✅ 친구 · 방 목록 · 가방 끝남 (2026-09-21, 커밋 `f780f01`)

둘이서 **신청 → 수락 → 놀러 가기**까지 확인했다. 설계 근거는 WORKLOG 19차.
`friend` 테이블은 게임 서버 소유, 한 관계를 **두 줄로**(A→B, B→A) 저장한다.

### ✅ 포스트잇 끝남 (커밋 `1da3b0d`)

**가구를 클릭할 수 있게 된 것**이 핵심이었다. 그 하나로 세 가지가 같이 열렸다:

| 클릭 | 동작 |
|---|---|
| 포스트잇 | 창 — 글쓴이·본문, 내 것이면 쓰기/떼기 (`C_PostitWrite`) |
| 화분 | 캣닢 꽂기 (`C_OfferItem`) — **남의 방에서도 된다** |
| 나머지 | 사용 (`C_UseItem`) — 의자·조명 등 |

**남의 방에 물건을 놓는 권한 예외는 포스트잇과 캣닢뿐**이라는 설계가 웹에서도 살아났다.

주의할 점:
- 포스트잇은 **벽걸이**다(`flags:["wall"]`, `rotations:[2,4]`). 클라가 `wallDirAt` 으로 방향을 골라 주지 않으면
  서버(`Walls.CanHang`)가 그냥 거절한다. 벽 없는 칸이면 **보내지 않고 안내**한다.
- 터치 판정은 도형(20px)보다 **넉넉히 44px** 로 잡았다. 안 그러면 폰에서 못 누른다.

### 🎨 받아야 할 그림 (GPT 요청 대기)

원본은 **`art/ref/files/`**. 받은 새 그림도 거기 넣고 `py tools/build-furni-tiles.py art/ref/files` 한 번,
그다음 `webapp/lib/game/furni.ts` 의 `FILES` 목록에 이름 한 줄 추가하면 끝난다.
(변환본 `webapp/public/art/furni/` 는 손대지 않는다. 첨부용으로도 쓰지 말 것 — 이미 작게 줄었다.)

**① 포스트잇 3색 ★가장 급함** — 기능은 됐는데 그림이 없어 네모로 나온다.
```
(2:1 아이소메트릭 픽셀 아트. 배경 완전 투명. 안티앨리어싱 없음.
 색 16개 이하. 그림자·받침대·라벨·글자 없음. 한 장에 하나만.
 파랑 #A0AFCE + 크림 #FCF0E1 위주의 아늑한 색.)

벽에 붙은 작은 정사각형 메모지 한 장.
살짝 기울어져 붙어 있고, 위쪽 모서리에 접힌 자국이 있다.
글씨는 쓰지 말 것 — 빈 메모지다.

[노란색 / 분홍색 / 민트색] 메모지.
```
파일명: `postit_yellow_4_blank.png` · `postit_pink_4_blank.png` · `postit_mint_4_blank.png`
`_written` 도 받으면 좋다(같은 메모지에 **글씨 몇 줄**). 그러면 읽을 게 있는지 방에 들어서자마자 보인다.
지금은 색만 바꿔서 표시하고 있다.

**② 상태가 짝이 없는 것 두 개** — 클릭해도 그림이 안 바뀐다(사라지진 않는다 — `furni.ts` 가 대체한다).
`art/ref/files/` 의 **원본을 첨부하고** "달라지는 건 ○○뿐" 이라고 해야 같은 그림이 나온다.

| 받을 파일 | 첨부할 원본 | 달라지는 것 |
|---|---|---|
| `lamp_floor_0_on.png` | `lamp_floor_0_off.png` | 갓 안쪽이 따뜻한 노란빛, 주변에 은은한 빛무리 |
| `fridge_red_0_open.png` | `fridge_red_0_closed.png` | 문이 왼쪽으로 열려 흰 선반과 우유·과일이 보임 |

**③ 침대** — 원본 시트에 없어 아직 못 만들었다. `art/ref/sprite-prompt.md` §3-2 ③ 프롬프트로.
...
**남의 방에서 활짝 핀 캣닢 화분을 클릭하면 캣닢 잎이 내 가방에 들어왔다** — 30장이면 1,500루피라,
남의 방을 도는 것만으로 돈이 생기는 구멍이었다(`OnUse` 에 주인 확인이 없었다).

막은 것은 **물건이 생기는 전이(`give_item:`)뿐**이다:

| | 남의 개인 방 | 공용 방 |
|---|---|---|
| 의자 앉기·조명 켜기 | ✅ 된다 (손해가 없고 같이 노는 재미) | ✅ |
| 캣닢 수확 (`give_item`) | ❌ "○○님의 것이에요" | — |
| 자판기 우유팩 (`give_item`) | ❌ | ✅ **된다** (무한 자판기는 설계) |
| 화분에 캣닢 꽂기 | ✅ 선물은 그대로 | 공용 방엔 화분이 없다 |

주인 판정은 **닉**으로 한다 — `OwnerId` 는 접속할 때 채워져서 주인이 없는 동안엔 믿을 수 없다.

### ✅ 마트(기획전) 공용방 (2026-09-26)

`data/rooms/mart.json` — 잠긴 공용 방. 지도의 "마트" 구역이 이걸로 살아난다.

| 클릭 | 열리는 것 |
|---|---|
| 진열대 (`shop_shelf`) | **그 상품만** 보이는 상점 창 |
| 계산대 (`shop_counter`) | 상점 **전체** (점원 고양이 = 가구 한 점. 진짜 NPC 는 경로·동기화가 붙어 규모가 커진다) |

**기획전 교체 = `mart.json` 의 `extra` 줄만 수정.** 진열대 정의는 하나고 상품만 갈아 끼운다 —
상품마다 가구를 만들지 않는다. 고친 뒤에는 `/admin/reload-defs`… 가 아니라 **서버 재시작**이 필요하다
(이미 만들어진 방은 생성 시점의 `RoomDef` 를 들고 있다 — `DefinitionStore.Reload` 주석 참고).

⚠ 진열대·계산대 그림이 아직 없어서 **도형으로 나온다.** 받을 파일: `shop_shelf_0_default.png` ·
`shop_counter_0_default.png` (프롬프트는 `art/ref/sprite-prompt.md` §2-3).

### ▶ 다음 후보

**운영자 대시보드** (그림 등록·설정) · 외모 바꾸기 · 이사/방 넓히기 · 팝플풍 HUD 디자인 · Godot 클라 정리(단계 7).
...
1. ✅ **로그인 속도 제한** (2026-09-26) — **각자 자기 문을 잠근다.**
   - 게임 서버: `LoginThrottle`(메모리) — 닉당 10분에 8회, 한 연결에서 5회 실패하면 끊는다.
     **IP 로는 안 센다** — 브라우저가 전부 cloudflared 를 거쳐 와서 서버 눈엔 다 127.0.0.1 이다(한 명 때문에 전원이 막힌다).
   - 웹 `/api/login`: `login_log` 를 세서 닉 8회 / IP 30회 per 10분. Vercel 은 인스턴스가 갈려 메모리 카운터를 못 쓴다.
     차단 기록(`detail='시도 제한'`)은 **세지 않는다** — 세면 새로고침할 때마다 스스로 잠금을 연장한다.
2. ✅ **공용 방 잠금** (2026-09-26) — `RoomDef.OpenEdit` 추가. 공용 방은 **기본이 잠김**이고,
   모래밭으로 열고 싶은 방만 템플릿에 `"openEdit": true` 를 넣는다. 지금 공용 방 둘(광장·댄스홀)은 잠겼다.
   → 댄스홀 자판기는 그대로 눌러 쓸 수 있다(그건 `UseItem` 이라 꾸미기 권한과 무관). **줍기만 막혔다.**
   ⚠ 이제 공용 방은 **아무도** 꾸밀 수 없다(운영자 포함) → 광장 배치를 바꾸려면 방 JSON 을 고친다.
