...
## 2026-09-18 (14차) — 선물 화분: 혼자서는 못 채우는 화분

사용자 의도를 뒤늦게 정확히 이해한 라운드. 원래 원했던 건 **"화분을 놓아두면 놀러 온 사람들이 채워 주고,
가득 차면 판다"** 였는데, 내가 만든 건 **시간이 지나면 자라는 혼자 하는 농사**였다. 사교가 통째로 빠져 있었다.
디자인 시트의 `1~5 / 6~15 / 16~29 / 30줄기(완성)` 은 시간 단계가 아니라 **받은 개수의 구간**이었다.

### 설계
- **배치가 아니라 상호작용.** 남의 방에 물건을 놓을 수 있는 건 여전히 **포스트잇뿐**이다.
  캣닢은 이미 놓인 화분에 대고 쓰는 것이므로 `C_OfferItem` 이라는 별도 요청으로 처리한다 — 꾸미기 권한을 건드리지 않는다.
- **혼자서는 못 채운다**(자기 화분 금지 · 공용 방 금지). 이게 이 기능의 존재 이유다.
- 숫자가 사교를 유도한다: 잎을 **혼자 팔면 40**, **남에게 주면 그 사람이 50**(1500÷30)을 받는다 → 주는 게 이득.
  자전거래는 장당 이득 10루피에 10분이 드는 데다 **같은 사람 하루 3개** 한도로 채산이 맞지 않는다.
- **인기도(fame)는 받은 누적 개수**로 따로 센다. 화분을 팔아도 줄지 않는다 → "팔까, 두고 자랑할까"가 진짜 고민이 된다.

### 구현에서 조심한 것
- `fame` 은 **세션이 아니라 SaveStore 가 유일한 기록자**다. 주인이 접속 중이 아니어도 올라가야 하므로,
  `UpdateUser` 는 비밀번호처럼 **기존 fame 을 보존**한다(세션이 모르는 값이다).
- 화분 단계는 저장된 상태 문자열이 아니라 **개수에서 다시 계산**한다(`Seed` 에서도). 용량을 바꿔도 따라온다.
- 하루 한도는 일부러 **메모리에만** 둔다(`GiftLimits`) — 목적이 봉쇄가 아니라 채산 악화이고, 요청마다 DB 를 치지 않기 위해서.
  영구 기록은 `gift_log` 가 남긴다.
- 상태명 `"empty"` 는 **빈 우유팩이 이미 쓰고 있었다** → 화분은 `"bare"` 로. 클라 `StateLabel` 이 switch 식이라 컴파일 에러로 바로 잡혔다.
- `Leaf` 는 `DrawPlant` 안의 지역 함수였다 → 화분이 둘이 되면서 공용 메서드로 승격.

### 남은 것
- 2인 이상 실제 플레이로 눈 확인(혼자서는 확인이 불가능한 기능이다).
- 화분을 클릭하면 **누가 꽂아 줬는지 목록** 보여주기(`gift_log` 는 이미 쌓인다).
- 외모 팔레트 스왑 셰이더(도트가 코드 그림을 덮어 색이 반영되지 않는 문제) — 진단만 끝났고 `tools/atlas-palette.py` 준비됨.
