...
## 2026-09-21 (19차) — 방 목록 · 가방 · 친구 ✅ 둘이서 만나고 놀러 가는 것까지 확인

### 링크를 줬는데 못 만났다

친구에게 링크를 주고 둘 다 들어왔는데 **서로 안 보였다.** 로그인하면 서버가 `homeRoomId`(내 집)를 주고
클라가 그리로 들어가기 때문이다. 각자 자기 집에 있으니 만날 수가 없었다.

단계 5를 **가방부터** 시작한 것이 순서 착오였다. 사람을 부를 수 있게 된 상태에서는
**만나는 기능이 먼저**다. 방 목록을 먼저 만들었어야 했다.

### 만든 것

**방 목록** — 공용 방(부둣가 광장·댄스홀)이 맨 위, **사람 많은 순**. 어디 가야 누가 있는지가 가장 궁금한 정보다.
"집" 섹션에는 저장된 개인 방이 다 나온다 — 주인이 접속하지 않아도 남아 있으므로(`RoomManager.RestoreSaved`)
놀러 가서 화분에 캣닢을 꽂아 둘 수 있다.

**가방** — 목록·판매(묶음이 이득이면 표시)·가구 놓기(고른 뒤 방 클릭, Esc 취소).

**친구** — 신청·수락·거절·삭제, 접속 표시, 놀러 가기.

### 친구 설계에서 정한 것

- **한 관계를 두 줄로 저장한다**(A→B, B→A). 한 줄이면 목록 쿼리마다 양방향 OR 이 붙는다.
  두 줄이면 `WHERE nick_key = 나` 하나로 끝나고, 신청한 쪽(`sent`)과 받은 쪽(`pending`)이 다른 상태를 가진다.
- **서로 신청하면 곧바로 친구.** 상대가 이미 신청해 둔 상태에서 내가 신청하면 수락으로 친다.
- **목록에 접속 여부·지금 있는 방을 함께 실어 보낸다.** 이게 없으면 목록이 이름표일 뿐이라
  놀러 가려면 방 목록을 또 뒤져야 한다. 오프라인이어도 **저장된 집**을 알려준다.
- **접속하면 친구들에게 알린다.** 접속 순간을 놓치면 만날 기회 자체가 없다 — 이 기능의 핵심이다.
- **`SaveStore` 에 넣지 않았다.** 지갑·가방은 매 틱 바뀌어 메모리가 진실이고 3초마다 내려쓰지만,
  친구는 하루 몇 번 바뀌고 **항상 최신이어야 한다**(상대가 수락한 걸 바로 봐야 한다). 목록도 짧다.
  더티 집합에 얹으면 복잡도만 늘고 얻는 게 없다 → `Data/Friends.cs` 가 DB 를 직접 본다.

### 프로토콜을 늘리며

`[Key]` 순서 함정을 하나 더 만났다. **`InventoryEntry` 는 `[id, 이름, 수량, …]` 인데
`S_InventoryUpdate` 는 `[id, 수량, 이름, …]`** 으로 1번과 2번이 뒤바뀌어 있다.
같은 읽기 함수를 쓰면 이름 자리에 숫자가 들어간다 — 역시 **오류 없이 조용히** 틀린다.

### 남은 것
- 웹 UI 는 아직 "기능 확인용"이다. Godot 의 `HudView.cs`(656줄)에 있던 상점·외모·이사가 없다.
- **디자인은 기능과 같이 잡는다.** 기준값은 `webapp/lib/ui/tokens.ts` 에 모아 뒀다(색은 `Ui.cs` → `palette48.json`).
- 모바일은 **열리긴 한다**(Pixi 의 `pointertap` 이 터치를 받는다). 다만 설계하지 않았다 — 확대/축소 없음, 좁은 화면 대응 없음.

## 2026-09-21 (18차) — 웹 클라이언트로 갈아타고 배포까지 ✅ 링크 하나로 들어간다

**https://myohang.vercel.app 에서 로그인 → 방 입장까지 동작.** 설치도 zip 도 없다.

```
브라우저 ──https──▶ Vercel (Next.js: 랜딩·로그인·게임)
         ──wss────▶ Cloudflare Tunnel ──▶ 내 PC :8080/ws ──▶ 방 루프
```

### 왜 웹으로 갔나

아침에 데스크톱 클라 배포 파이프라인을 **완성하고 나서** 결정했다. 70MB zip, 압축 위치, `harbor://` 등록,
SmartScreen 경고, dll 누락 — 테스터가 넘어야 할 턱을 직접 다 만들어 보고 나니 답이 분명해졌다.
웹이면 링크 하나다. 그리고 **설치가 없으니 업데이트도 없다** — 버전 체크 화면이 통째로 필요 없어졌다.

### 무엇을 버렸나: 클라이언트만

서버·DB·프로토콜·아트·방 템플릿은 **한 줄도 안 버렸다.** `Session` 에서 전송만 `ITransport` 로 떼어내니
TCP(Godot)와 WebSocket(브라우저)이 **같은 세션·핸들러·방 루프**를 지난다. 웹 클라를 붙이는 동안
서버에 게임 로직이 한 줄도 추가되지 않았다.

| 단계 | 내용 |
|---|---|
| 0 | `/ws` — `ITransport` 로 전송 분리 |
| 1 | TS 프로토콜(`framing`·`opcode`·`packets`) · 접속 · 로그인 |
| 2 | PixiJS 방 렌더링 + 바닥 6종 + 벽 5종 |
| 3 | 이동 — 클릭 · 화살표 · WASD, 도트 캐릭터 |
| 4 | 채팅 — 말풍선(Pixi Text) + 로그(React) |

### 프로토콜을 옮기며 배운 것

**필드 이름은 전선에 실리지 않는다.** `[MessagePackObject]` + `[Key(n)]` 은 객체를 **배열**로 직렬화한다.
`C_Login { Login, Token }` 은 `["닉","토큰"]` 이다. 이름으로 맞추려 들면 전부 `undefined` 가 되는데
**오류가 나지 않고 빈 화면**이 된다. 그래서 `packets.ts` 는 자리(index)로 읽고 쓴다.
→ `Packets.cs` 의 `[Key]` 번호는 계약이다. **새 필드는 반드시 맨 뒤 번호로.**

### 방향 매핑 — 세 번 틀리고 나서야 계산으로 풀었다

두 번은 "왼쪽이 반대네" → 뒤집고, "이제 오른쪽이 반대" → 다시 뒤집는 식으로 **관찰에서 역산**했다.
어느 쪽을 뒤집어도 한쪽이 틀렸다. 전제가 틀렸기 때문이다.

그림을 확대해 보니 시트는 `0 = 뒤통수 … 4 = 정면`이고 사이는 **왼쪽을 보며** 돈다(얼굴이 왼쪽에서 드러난다).
그리고 결정적으로 — **카메라가 45° 돌아가 있다.** 투영이 `sx=(x-y)·32, sy=(x+y)·16` 이므로
`dir 7` 이 화면에서 곧게 위(가장 멀다 = 뒷모습)이고 `dir 3` 이 곧게 아래(가장 가깝다 = 정면)다.
**시트 번호는 `7 - dir` 로 읽어야 한다.** Godot 의 `DirOffset = 1` 은 여기서 틀렸다(45°만 돌리고 회전 방향을 놓쳤다).

**교훈: 그림 문제는 그림을 보고 푼다.** 움직임을 보고 역산하면 두 번까지는 그럴듯하게 맞는다.

### 아트 파이프라인

- **바닥 6종** — 정확히 64×32, 모서리 투명, 색 4~5개로 받아 **그대로 투입**. 손댈 게 없었다.
- **벽 5종** — 받은 것은 '벽 한 판 전체'(윗면·옆면 두께 + 걸레받이)였다. 게임은 타일 한 칸씩 이어 붙이므로
  그대로 반복하면 옆면 두께가 32px마다 나타난다. → **정면 무늬만** 잘라 32×48 반복 타일로.
  높이 48인 이유: 벽이 3칸(96px)이라 **정확히 2번** 반복된다. 40이면 2.4번이라 중간에 이음매가 보인다.
  격자 복원은 실패했다 — 최빈 런이 2~6으로 흩어졌다. 고양이와 달리 **처음부터 고해상도로 그린** 그림이라
  복원할 격자가 없었다. LANCZOS 축소 + 16색 양자화로 갔고 결과는 깨끗하다. → `tools/build-wall-tiles.py`

### 하루에 같은 모양의 실패를 네 번 겪었다

전부 **"실패했는데 화면은 멀쩡해 보이는"** 종류였다. 오류 메시지가 없어서 매번 한참 걸렸다.

| 증상 | 진짜 원인 | 어떻게 잡았나 |
|---|---|---|
| 고양이가 네모 | `atlas.json` 이 pck 에 들쭉날쭉 | 프레임 표를 **dll 상수**로 이중화 |
| 검사가 항상 통과/항상 실패 | 문자열 존재만 봄 · .NET 은 **UTF-16** 저장 | `tr -d '\000'` 후 grep, 값(52)으로 확인 |
| 웹앱 배포만 실패 | `npm run dev` 는 **타입 검사를 건너뛴다** | `npm run build` 를 커밋 전에 |
| **빌드 성공인데 전 경로 404** | **`Framework Preset` 이 `Other`** | 빌드 로그로는 알 수 없다 — **설정 화면을 봐야 했다** |

마지막 것이 제일 오래 걸렸다. 빌드 로그는 `Build Completed [31s]`, `Deployment completed` 로 완벽했고
네 경로가 다 생성됐는데도 전부 404였다. `landing` 이 정적 사이트였던 시절의 설정이 남아 있어
Vercel 이 Next.js 앱을 정적 사이트로 취급했고, 그래서 **라우팅을 만들지 않았다.**
Root Directory 만 확인하고 Framework Preset 은 확인하지 않은 것이 화근.

### 남은 것
- 로그인 **속도 제한** — 공개 전 필수. 링크를 공개된 곳에 올리지 말 것
- 공용 방 잠금(`CanEdit` 이 public 이면 무조건 true)
- 터널 주소는 `cloudflared` 재시작마다 바뀐다 → 고정하려면 계정 + 도메인


## 2026-09-18 (17차) — 테스터에게 줄 클라이언트 exe ✅ 압축 풀어 실행까지 확인

`bash tools/pack-client.sh` 한 줄로 **내보내기 → 검사 → 동봉 → 압축**까지 끝난다. 산출물 `myohang-client.zip` (70MB).

### 막혔던 것 세 가지

1. **`.sln` 이 없어서 내보내기가 아예 안 됐다.** Godot 의 .NET 내보내기는 `project.godot` 옆에서
   `<assembly_name>.sln` 을 찾는다. 우리 솔루션은 저장소 루트에만 있었다(개발은 `dotnet build` 로 해와서 몰랐다).
   → `client-godot/HarborClient.sln` 추가. Godot 은 **`ExportRelease`** 구성으로 빌드하는데 `Harbor.Core`·
   `Harbor.Protocol` 에는 그 구성이 없으므로, 솔루션에서 **`Release` 로 이어 주는 매핑**이 반드시 있어야 한다.

2. **내보내기 폴더를 Godot 이 만들어 주지 않는다.** 없으면 "주어진 내보내기 경로는 존재하지 않습니다". → `mkdir -p` 먼저.

3. **`atlas.json` 이 pck 에 들어갔다 말았다 했다.** ← 오늘 가장 위험했던 것

### atlas.json 문제와 그 해법 (프레임 표를 dll 상수로 이중화)

`atlas.json` 은 Godot 이 말하는 '리소스'가 아니라, `export_presets.cfg` 의 `include_filter` 에 걸려야만 pck 에 들어간다.
그런데 **같은 설정으로 내보내도 결과가 달라졌다** — `art/atlas.json` 로 지정했을 때 pck 가 63,464 바이트(포함)였다가,
다음 실행에서 32,328 바이트(누락)로 돌아왔다. 원인은 끝내 특정하지 못했다.

문제는 빠졌을 때의 증상이다. **크래시가 아니라 `AssetCatalog` 가 placeholder 모드로 조용히 떨어져 고양이가 네모로 나온다.**
설정 하나에 아트 전체가 걸려 있고, 실패해도 티가 안 난다 → **파일 의존 자체를 없애는 쪽**으로 갔다.

- `tools/build-avatar-atlas.py` 가 `atlas.json` 과 함께 **`art/AtlasMeta.cs`**(같은 JSON 을 C# 상수로)를 뱉는다.
- `AssetCatalog` 는 **파일이 있으면 파일**(에디터에서 아틀라스를 다시 만들면 빌드 없이 즉시 반영),
  **없으면 상수**(내보낸 게임)를 읽는다. 스크립트는 dll 로 컴파일되므로 빠질 수가 없다.
- 확인: 배포본을 저장소 밖에 풀어 실행 → 고양이 정상 출력.

### 검사식을 두 번 잘못 썼다 (기록해 둘 가치가 있다)

포장 스크립트에 넣은 자동 검사가 연달아 틀렸다. **검사식을 먼저 검증하지 않고 파이프라인에 넣은 것**이 원인이다.

| 시도 | 결과 | 왜 틀렸나 |
|---|---|---|
| `grep -aq "art/atlas.json" myohang.pck` | **항상 통과** | 파일 포함 여부가 아니라 문자열 존재만 봄. 누락된 pck 도 통과했다 |
| `grep -aq "avatar/hd/" HarborClient.dll` | **항상 실패** | .NET 은 문자열 상수를 **UTF-16** 으로 저장한다(`a\0v\0…`) |
| `tr -d '\000' < dll \| grep -ac "avatar/hd/"` | ✅ 52개 | 널바이트를 걷어내면 ASCII 가 된다. `grep -P` 는 이 로케일에서 거부당한다 |

**자동 검사는 "통과하는 것"과 "실패하는 것"을 둘 다 눈으로 본 뒤에 파이프라인에 넣는다.**

### 테스터 동봉물
`tools/client/` 에 테스터용 `harbor://` 등록 스크립트와 `읽어주세요.txt`. 기존 `tools/register-url-scheme.ps1` 은
**개발자용**이라(에디터 실행 파일 + 소스 폴더를 가리킨다) 테스터에게 주면 안 된다.

---

## 2026-09-18 (16차) — 웹 로그인: Vercel에서 로그인하고 게임에 입장한다 ✅ 실제 동작 확인

브라우저는 **Vercel 만** 부르고, 게임 서버는 **서명만 검증**한다. 둘은 서로 통신하지 않는다.

```
브라우저 → Vercel /api/login → Supabase(계정 확인) → 서명된 입장권
                                                        ↓
                             게임 클라 ──TCP──▶ 내 PC 게임 서버 (공유 키로 서명 검증)
```

### 왜 이 구조인가
- 전에는 입장권이 **게임 서버 메모리**(`TicketStore`)에 있어서, 발급자와 검증자가 같은 프로세스여야 했다 → 웹을 떼어낼 수 없었다.
- **HMAC 서명 입장권**(`t1.<payload>.<sig>`, payload = `닉|만료`)으로 바꾸니 발급은 어디서든 되고 검증은 오프라인으로 된다.
- 덤으로 **https→http 혼합 차단이 사라진다** — 브라우저가 내 PC 서버를 직접 부를 일이 없기 때문이다.
- 비밀번호 규격(PBKDF2-SHA256 · 120,000회 · 소금16 · 해시32 · base64)을 Node 쪽에서 **그대로** 재현해 같은 계정을 공유한다.

### 실제로 걸린 것 (전부 진단 도구로 잡았다)
1. **접속 문자열 형식이 다르다** — 게임 서버는 Npgsql(`Host=…;Port=…`), Node `pg` 는 URI(`postgresql://…`).
   값을 두 벌로 관리하면 반드시 어긋나므로 **둘 다 받아들이게** 했다(`landing/api/_db.js`).
2. **환경변수가 조용히 비어 있었다** — 이름은 있는데 값이 빈 문자열. 화면엔 그냥 "설정이 빠졌어요"로만 보였다.
3. **값에 공백·줄바꿈이 붙는다** — `HARBOR_GAME_PORT` 가 `"30000\n"`(6자)이었다. 이제 항상 `trim` 한다.
4. **Vercel 은 환경변수를 배포 시점에 주입한다** — 나중에 추가한 값은 기존 배포에 반영되지 않는다. 반드시 **Redeploy**.

### 그래서 만든 것: `/api/health`
값을 **절대 돌려주지 않고** 있는지·길이·공백 여부만 알려 준다. `?db=1` 을 붙이면 **DB 연결까지 시험**해
붙은 호스트·포트와 실패 코드(`ENOTFOUND`/`ETIMEDOUT`/`28P01`…)를 준다.
추측으로 세 번 헤매던 것을 이 한 번으로 끝냈다 — **비밀을 노출하지 않으면서 진단 가능하게** 만드는 것이 요점.

### 키 다루기에서 한 실수
서명 키를 만들면서 `py -c "…print(token_hex(32))"` 로 **화면에 찍었다** → 대화 기록에 남았다. 폐기하고
`tools/new-secret.py` 로 다시 만들었다(값을 출력하지 않고 `.env` 에 바로 쓴다). `CLAUDE.md` 에 규칙으로 박았다.
**`HARBOR_TICKET_SECRET` 은 DB 문자열과 같은 급이다** — 이 키를 알면 아무 닉으로나 입장권을 만들 수 있다.

### 확인된 것
- 코드 복사 → 게임 비밀번호 칸 붙여넣기 → **입장 성공**
- **[게임 바로 열기]** 버튼 → `harbor://` 로 클라이언트가 뜨고 **그대로 입장** (이 PC 는 프로토콜이 등록돼 있다)

### 남은 것
- **포트포워딩** — 지금 `HARBOR_GAME_HOST` 가 `127.0.0.1` 이라 이 PC 에서만 된다. 외부 테스터는 공개 주소가 필요하다.
- **테스터 PC 에는 `harbor://` 가 없다** → 클라 zip 에 `tools/register-url-scheme.bat` 를 넣고 "처음 한 번 실행" 안내를 붙인다.
  안 해도 코드 붙여넣기로는 들어갈 수 있다.
- Vercel 쪽 DB 는 **6543(트랜잭션 풀러)** 가 서버리스에 맞다(지금 5432 로 붙어 있고, 동작은 한다).
- 서명 입장권은 만료 전 **재사용이 가능**하다(메모리 입장권과 달리). 5분짜리라 감수하되, 문제가 되면 nonce 기록을 추가한다.

## 2026-09-18 (15차) — 외모 색이 도트에 먹히게 (팔레트 스왑 셰이더)

### 문제는 버그가 아니라 미완성이었다
도트가 들어온 뒤로 외모를 바꿔도 화면이 그대로였다. 원인은 `AvatarView._Draw()` 의 첫 줄
`if (!_placeholder) return;` — **도트가 있으면 코드 그림을 아예 안 그린다.** 그런데 털색·옷색을 쓰던 곳이
바로 그 코드 그림이었다. 12차에 `spr.SetMeta("palette", …)` 로 자리만 잡아 두고 미뤘던 작업이 드러난 것.

### 왜 이 그림에서 팔레트 스왑이 잘 되는가
`tools/atlas-palette.py` 로 뽑아 보니 **색이 14개뿐이고 안티앨리어싱이 없다.** 즉 털에 쓰인 색이 딱 그 값으로만
존재하므로, **정확히 일치하는 색만 갈아끼우면** 경계가 뭉개지지 않는다. 그리고 그 14색이
`art/ref/sprite-prompt.md` 에 적어 둔 지정 팔레트와 정확히 일치했다 — 프롬프트에 색을 박아 둔 값이 여기서 돌아왔다.

### 한 축으로는 부족했다 (사용자 지적)
처음엔 털 9색을 한 덩어리로 묶었다. 그랬더니 색 하나가 몸 전체를 덮어 **"내 고양이를 닮게" 만들 수가 없었다.**
그래서 그룹을 쪼갰다:
...
흰 바탕 + 검정 무늬 = 젖소, 크림 + 주황 = 치즈 태비. **조합이 생겼다.**
각 팔레트의 **1번을 원화 색**으로 맞춰서, 아무것도 고르지 않은 기본 상태가 그림 그대로다.

### 명암을 지키는 방법 — 비율이 아니라 사다리
처음엔 밝기를 비율로 옮겼다(`target.V × src.V / base.V`). 밝은 색에서는 괜찮지만 **검정을 고르면 9단계가 전부
바닥에 몰려 한 덩어리**가 된다. 그래서 그룹 안의 **상대 위치(가장 어두운 색 0 ~ 밝은 색 1)** 를 구하고,
목표색을 어둡게/밝게 한 양 끝 사이에서 그 위치를 찍는 방식으로 바꿨다 — 어떤 색을 골라도 단계가 남는다.

### 못 한 것과 그 이유
- **눈 색** — `5b4854` 를 바꿔도 화면에 변화가 없었다. 눈동자가 **외곽선과 같은 색**으로 찍혀 있어서,
  그 색을 바꾸면 얼굴 선까지 물든다. **그림 쪽 문제**라 `art/ref/sprite-prompt.md` §2-2 에 규칙으로 적었다:
  **"눈동자를 외곽선과 같은 색으로 찍지 말 것."** 색이 곧 조절 손잡이다.
- 무늬 모양(턱시도·얼룩·젖소)·모자·하의 — 해당 그림 자체가 없다. UI 에서는 "아직 반영되지 않음" 아래로 내렸다
  (고른 값은 저장되므로 그림이 들어오면 그대로 살아난다).

### 그 밖에
- 머티리얼은 **아바타마다 따로** 만든다(공유하면 방 안 모두가 같은 색이 된다).
- `Figure.Parts` 에 `ea`(귀)를 **뒤에 추가**. 옛 저장본은 그 항목이 없을 뿐이라 그대로 유효하고, 서버는 형식만 검사한다.
- 기본 외모의 `hd` 팔레트를 2(치즈) → **1(크림)** 로. 아무것도 안 고른 상태가 원화와 같아진다.
